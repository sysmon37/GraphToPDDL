...
def processLayer3(graphUAG):
    ...
    gbase_diff, gopt_diff = process(graphUAG)



def process(graphUAG):
    """
    ...
    #Get G_base
    graph_G_base = get_treatment_graph(graphUAG, 'base')
    #Get G_opt
    graph_G_opt = get_treatment_graph(graphUAG, 'opt')
...
def get_treatment_graph(unionAG, which_plan):
    """
    ...
    Input Args:
        ...
        which_plan: this takes on one of two values: 'base' denoting the base
        treatment plan or 'opt' denoting the optimal treatment plan
    ...
    if which_plan == 'base':
        nodelist1 = parseTreatmentPlan('base')
        subg1 = nwx.subgraph(unionAG, nodelist1)
        return subg1
    else:
        if which_plan == 'opt':
            nodelist2 = parseTreatmentPlan('opt')
            subg2 = nwx.subgraph(unionAG, nodelist2)
            return subg2
        else:
            return
...
def parseTreatmentPlan(which_plan):
    """
    ...
    if which_plan == 'base':
        plan_output = open('Plans/baseplan.txt', 'r')
        #plan_output = open('Plans/baseplanold.txt', 'r')
    else:
        if which_plan == 'opt':
            plan_output = open('Plans/optplan.txt', 'r')
            #plan_output = open('Plans/optplanold.txt', 'r')
        else:
            return
